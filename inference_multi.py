  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      for j, logits in enumerate(logits_split):  #
          ...
          accuracy += tf.cast(update_op_accuracy, dtype=tf.float64)
      accuracy = accuracy / tf.constant(FLAGS.num_aspects, dtype=tf.float64)

      logging_hook = tf.train.LoggingTensorHook({"loss": total_loss,'accuracy':accuracy,'predictions':predictions,'label_ids':label_ids}, every_n_iter=5)
...
def set_flags_ss(flags):
    ...
    GLUE_DIR='multi_labels_data'

    flags.task_name='sentiment_analysis'
    flags.do_train=True
    flags.do_eval=True
    # flags.do_predict=True
    flags.num_classes=8
    flags.num_aspects=4
    flags.aspect_value_list=[0,1]
    ...
    flags.output_dir='tmp/ss_output/muti_labels'
    return flags

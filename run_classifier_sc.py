...
class MnliProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    ...
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, "train0.tsv")), "train")
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
                 labels, num_labels, use_one_hot_embeddings,is_real_example):
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    (total_loss, per_example_loss, logits, probabilities) = create_model(
        ...
        num_labels, use_one_hot_embeddings,is_real_example)
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      predictions = tf.argmax(logits, axis=-1, output_type=tf.int32)
      accuracy,acurr_op = tf.metrics.accuracy(
          labels=label_ids, predictions=predictions, weights=is_real_example)

      logging_hook = tf.train.LoggingTensorHook({"loss": total_loss,'accuracy':acurr_op,'predictions':predictions,'label_ids':label_ids}, every_n_iter=5)
...
def set_flags_ss(flags):
    ...
    BERT_BASE_DIR='/Users/user/code/chinese_L-12_H-768_A-12'

    print(os.path.abspath(BERT_BASE_DIR))
    GLUE_DIR='/Users/user/code/bert/my_data'
    ...
    # flags.vocab_file=BERT_BASE_DIR+'/vocab.txt'
    flags.vocab_file = '/Users/user/code/my_bert/voice_keys_check/hanzi_sc_res.dic'
    flags.bert_config_file=BERT_BASE_DIR+'/bert_config.json'
    # flags.init_checkpoint=BERT_BASE_DIR+'/bert_model.ckpt'
    ...
    flags.learning_rate=2e-5
    flags.num_train_epochs=30
    flags.output_dir='tmp/ss_output1/'
    return flags
...
def infer():
  ...
  (total_loss, per_example_loss, logits, probabilities) = create_model(
     ...
     num_labels, FLAGS.use_tpu,is_real_example)
  ...
  sess=tf.Session()
  ckpt = tf.train.latest_checkpoint(FLAGS.output_dir)
  saver.restore(sess, FLAGS.init_checkpoint)  # 加载到当前环境中
  def pred(text):
      example = InputExample(guid=0, text_a=text, text_b=None, label='1')
      feature = convert_single_example(0, example, label_list,
                                         FLAGS.max_seq_length, tokenizer)
      feed_list = {input_ids: [feature.input_ids], input_mask: [feature.input_mask],
                     segment_ids: [feature.segment_ids], label_ids: [[feature.label_id]]}
      predictions = tf.argmax(logits, axis=-1, output_type=tf.int32)
      predictions = sess.run([predictions,probabilities], feed_dict=feed_list)
      prob=predictions[1][0]
      prob=[round(val,6) for val in prob]
      print(predictions[0])
      print(prob)
